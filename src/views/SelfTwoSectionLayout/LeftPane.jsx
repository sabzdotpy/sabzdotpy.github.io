...
export default function LeftPane({ className }) {
    ...
    return (
      <>
        ...
        `}
        >
          <h4 className="mb-6">{greetings[Math.floor(Math.random() * greetings.length)]}</h4>

          <img 
            src={logo} 
            alt="Headshot"
            className="h-36 w-36 rounded-full border-2 border-dashed border-(--color-primary) object-cover" 
          />
          <h2 className="m-0 mt-6 mb-0 text-(--color-primary) font-[Playfair_Display]">Sabari S</h2>
